...
![](./image/Http_Response.png)
	...
	[Response]响应的组成结构：
		...
		a).状态行：包含协议版本号、响应状态码、状态码描述。
			...
			200——请求成功。
			3XX——重定向。
			403——服务器收到请求，但拒绝进行处理。404——资源不存在。
			500——服务器发生不可预期的错误。

		b).响应报头：键值对,附加响应信息
			如Content-Type:告诉客户端，响应的内容类型
			Date: 生成消息的具体时间和日期，即当前的 GMT 时间   等...
...
###5、 MVC 和 MVP 模式
	MVC 模式:
		...
		在 Android 中，分工如下:
			...
			b)View视图：也叫表示层，与用户实现交互的界面//布局文件。
...
![](./image/mvc_mvp.jpg)
	...
	MVP 模式：
		基于 MVC 模式改进得到的三层架构模式，旨在解决 MVC 的造成 Controller层臃肿的问题。
